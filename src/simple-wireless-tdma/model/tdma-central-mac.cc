...
{
  if (m_queue->GetMaxSize () == m_queue->GetSize (false))
    {
      return 0;
    }
  else
...
void
TdmaCentralMac::StartTransmission (uint64_t transmissionTimeUs, bool isCtrl)
{
  NS_LOG_DEBUG (transmissionTimeUs << " usec");
  Time totalTransmissionSlot = MicroSeconds (transmissionTimeUs);
  
  if (m_queue->IsEmpty (isCtrl))
    {
  ...
  Ptr<const Packet> peekPacket = m_queue->Peek (&header,isCtrl);
...
void
TdmaCentralMac::SendPacketDown (Time remainingTime, bool isCtrl)
{
  WifiMacHeader header;
  Ptr<const Packet> packet = m_queue->Dequeue (&header,isCtrl);
  m_low->StartTransmission (packet, &header);
  TxQueueStart (0);
  NotifyTx (packet);
  TxQueueStart (0);
  StartTransmission (remainingTime.GetMicroSeconds (),isCtrl);
}
...
void
TdmaCentralMac::FlushQueue(void) const
{
  m_queue->Flush();
}
