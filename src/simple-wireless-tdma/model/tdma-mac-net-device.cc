...
bool
TdmaNetDevice::SendbyMac (Ptr<Packet> packet, Mac48Address realTo, uint16_t protocolNumber)
{
  NS_LOG_FUNCTION (*packet << " Dest:" << realTo << " ProtocolNo:" << protocolNumber);
//  Mac48Address realFrom = Mac48Address::ConvertFrom (GetAddress ());
  LlcSnapHeader llc;
  llc.SetType (protocolNumber);
  packet->AddHeader (llc);
  //m_mac->Enqueue (packet, realTo, realTo);
  m_mac->Enqueue (packet, realTo);
  return true;
}
