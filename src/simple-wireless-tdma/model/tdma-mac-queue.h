...
public:
   ...
   */
  Ptr<const Packet> Dequeue (WifiMacHeader *hdr, bool isCtrl);
  Ptr<const Packet> Peek (WifiMacHeader *hdr, bool isCtrl);
  /**
...
private:
  ...
  PacketQueue m_queue[2]; // 0 is data queue, 1 is Ctrl/Entry queue
  uint32_t m_size[2];
  uint32_t m_maxSize;
  Time m_maxDelay;
  uint32_t m_count[2];
